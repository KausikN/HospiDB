 ...
 * @author Kausik N
 */
public class Table_UpdatePageEnter extends javax.swing.JFrame {

    /**
     * Creates new form LoginPage
     */
    public Table_UpdatePageEnter() {
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        jScrollPane3.setBounds(730, 160, 500, 450);
        ...
        jScrollPane4.setBounds(270, 160, 420, 450);
...
void UpdateList(){
    ...
    while(rs.next()){
        ...
        for(int i=0;i<columns_in_table_size;i++){
            ...
            if(Selected_Ids[0].equals(rs.getString(1))){  //REMEMBER SQL INDEX STARTS FROM 1
                dtmenter.addRow(row);
            }
        }
...
void MySQLConnector_Update(){
    ...
    for(int i=1;i<columns_in_table_size;i++){
        s = "update "+Table+" set "+columns_in_table[i]+" = "+update_values[i]+" where "+columns_in_table[0]+" = '"+Selected_Ids[0]+"';";System.out.println(s);
        st1.executeUpdate(s);   
    }
    con.close();
    }
...
void RowSizeFitter(){
    System.out.println(column_list.getSize().height);
...
DefaultTableModel dtmenter = (DefaultTableModel)enter_values.getModel();
for(int i=0;i<columns_in_table_size;i++){
    if(dtmenter.getValueAt(i, 0) != null)update_values[i] = dtmenter.getValueAt(i, 0).toString();
    else update_values[i] = "";
    update_values[i] = "'"+update_values[i]+"'";
}
...
Table_UpdatePageEnter.this.setVisible(false);
h.setVisible(true);
